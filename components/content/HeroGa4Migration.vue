<template>
  <section>
    <div class="white">
      <div class="container pt-15 md:pb-10 lg:pt-20">
        <h1 class="text-gradient text-center font-semibold text-3xl md:text-[35px] lg:text-5xl">
          <ContentSlot :use="$slots.title" unwrap="p" />
        </h1>
        <div class="max-w-[1000px] mx-auto">
          <video v-if="locale === 'en'" controls controlslist="nodownload" :poster="posterSrc" class="rounded-lg">
            <source :data-src="videoSrc" :src="videoSrc" type="video/mp4">
          </video>
          <nuxt-img v-if="locale === 'ja'" src="/analyze.png" format="webp" />
        </div>
        <div class="md:flex md:items-center md:gap-x-8">
          <div class="text-gray-500 text-center md:max-w-[650px] lg:max-w-[900px] mx-auto prose my-10 md:flex-1  md:my-0 lg:text-lg">
            <div class="md:block" :class="{hidden: locale === 'ja'}">
              <slot name="description" />
            </div>
            <div class="md:hidden">
              <slot name="mobileDescription" />
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script lang="ts" setup>
const { locale } = useI18n()

const videoSrc = computed(() => {
  if (locale.value === 'en') {
    return '/en/videos/GA4-migration-video.mp4'
  } else {
    return '/videos/GA4-migration-video.mp4'
  }
})

const posterSrc = computed(() => {
  if (locale.value === 'en') {
    return '/en/ga4-migration.webp'
  } else {
    return '/ga4-migration.webp'
  }
})
</script>

<style scoped>
.bg-gradient-head {
  background-image: linear-gradient(101.87deg, #2D92E9 4.15%, #644AD5 22.36%, #C141AC 41.56%, #E72192 61.75%, #F04D66 78.49%, #FD7043 98.68%);
}
</style>
