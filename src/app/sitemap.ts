...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const blog = getAllPosts(defaultLocale).map((post) =>
    post.url.replace(`${defaultLocale}/blog`, "/blog")
  );
  const tags = TAGS.map((tag) => `/blog/tag/${tag}`);

  const slugsMerged = [
    ...flattenAndMergeSlugs(product),
    ...flattenAndMergeSlugs(useCases),
    ...slugs,
    ...blog,
    ...tags,
  ];
  ...
  return {
    ...
    alternates: {
      languages: Object.fromEntries(
        locales.map((locale) => [locale, getUrl(host, pathname, locale)])
      ),
    },
  };
}
