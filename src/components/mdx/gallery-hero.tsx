...
const GalleryHero: React.FC<Props> = ({
  src,
  FormTitle,
  children,
  variant = "primary",
              ...
              <div className="w-full lg:w-2/3 prose">
                <div className="flex flex-col sm:flex-row items-center gap-1 ">
                  <div className="w-full sm:w-2/3">
                    {description && (
                      <Description className="textp">{description}</Description>
                    )}
                    {otherChildren}
                  </div>
                  <div className="w-full sm:w-1/3">
                    <Image
                      src={src}
                      alt={titles?.[0]?.props?.children}
                      width={300}
                      height={400}
                      className="my-auto md:h-150 w-full rounded-lg object-cover mb-8"
                    />
                  </div>
                </div>
              </div>
          ...
          <div className="rounded-lg max-w-[1050px] mx-auto p-5 lg:p-10">
            <div>
              ...
              <div className="md:text-center">
                {subtext && (
                  <Subtext className="text-primary-600 font-bold">
                    {subtext}
                  </Subtext>
                )}
                {titles.map((child, index) => (
                ...
                <div className="relative max-w-[380px] md:max-w-[500px] mx-auto mt-10">
                  {videoSrc ? (
                    <Video
                      ...
                      width={340}
                      height={600}
                      className="my-auto md:h-150  rounded-lg object-cover mb-8"
                    />
                  ) : (
                    <Image
                      src={src}
